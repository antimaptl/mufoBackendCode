...
class Login(APIView):
    ...
    def post(self, request):
        ...
        # Check if user already exists in all profiles
        ...
        if not profile:
            # ✅ Check if user already exists in User model by phone
        ...
        # If profile already found in list, send existing UID
        return Response({
            'uid': str(profile.uid),
            'otp': str(profile.otp),
            'message': "OTP sent to existing profile"
        })
